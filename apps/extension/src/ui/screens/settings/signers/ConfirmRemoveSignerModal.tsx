...
export function ConfirmRemoveSignerModal({
  isOpen,
  signerName,
  busy,
  error,
  ...
  const displayName = signerName.trim() || 'this passkey'
  ...
  return createPortal(
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
      ...
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="remove-signer-modal-title"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex w-full flex-col gap-3">
          <div className="flex flex-col gap-2 text-center">
            <h2
              id="remove-signer-modal-title"
            ...
            <p className="text-base leading-[1.36] tracking-[-0.32px] text-[#b3b3b3]">
              <span className="font-bold text-[#fcfcfc]">{displayName}</span> will no longer be able
              to sign for this wallet, and it will stop working for recovery. You will approve this
              with your current passkey.
            </p>
          </div>
          {error ? (
